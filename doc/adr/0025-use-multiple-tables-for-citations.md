...
We want to create new tables to better organize the references. For each type of reference (e.g. caselaw decision, dependent litrature, ...) we create the following tables:
* `citation_{type}_link`
    * references both the target and source using an identifier
...
blind-links with an existing target in a post-processing step to links. Afterwards, we remove active and passive
