...
public class TextCheckController {
  ...
  public ResponseEntity<TextCheckResponse> checkCategory(
    ...
    try {
      matches = textCheckService.checkCategory(id, CategoryType.forName(category));
    } catch (DocumentationUnitNotExistsException ex) {
      return ResponseEntity.internalServerError().build();
    }

    return ResponseEntity.ok(TextCheckResponseTransformer.transformToDomain(matches));
  }
}
