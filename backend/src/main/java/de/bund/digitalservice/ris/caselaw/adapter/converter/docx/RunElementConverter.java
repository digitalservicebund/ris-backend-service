...
public class RunElementConverter {
  ...
  private static void parseCTImageData(
    ...
    // imageElement.setBytes(image.bytes());
    imageElement.setPath(image.path());
  ...
  private static RunElement parseGraphicData(
    ...
    if (pic != null) {
      ...
      if (image != null) {
        addImageContent(imageElement, image, size, image.path());
      }
    } else {
  ...
  private static void addImageContent(
      InlineImageElement element, DocxImagePart image, Dimension size, String id) {
    ...
    if (image.contentType().equals("image/unknown")) {
      ...
      // TODO set bytes for unknown image type
    } else {
