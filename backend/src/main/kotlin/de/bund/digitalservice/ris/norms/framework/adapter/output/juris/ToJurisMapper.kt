...
fun mapDomainToData(norm: Norm): NormData {
  return NormData(
      officialLongTitle =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.OFFICIAL_LONG_TITLE)
              .firstOrNull(),
      risAbbreviation =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.RIS_ABBREVIATION)
              .firstOrNull(),
      risAbbreviationInternationalLawList =
          extractSimpleStringValuesFromNormSection(
              norm, MetadatumType.RIS_ABBREVIATION_INTERNATIONAL_LAW),
      unofficialLongTitleList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.UNOFFICIAL_LONG_TITLE),
      unofficialShortTitleList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.UNOFFICIAL_SHORT_TITLE),
      unofficialAbbreviationList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.UNOFFICIAL_ABBREVIATION),
      divergentDocumentNumber = extractFirstDivergentDocumentNumber(norm),
      documentCategory =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.DOCUMENT_CATEGORY)
              .firstOrNull(),
      ...
      officialShortTitle =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.OFFICIAL_SHORT_TITLE)
              .firstOrNull(),
      officialAbbreviation =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.OFFICIAL_ABBREVIATION)
              .firstOrNull(),
      ...
      referenceNumberList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.REFERENCE_NUMBER),
      definitionList = extractSimpleStringValuesFromNormSection(norm, MetadatumType.DEFINITION),
      validityRuleList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.VALIDITY_RULE),
      ...
      ageOfMajorityIndicationList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.AGE_OF_MAJORITY_INDICATION),
      ageIndicationStartList = extractAgeIndicationStarts(norm),
      celexNumber =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.CELEX_NUMBER).firstOrNull(),
      ...
      unofficialReferenceList =
          extractSimpleStringValuesFromNormSection(norm, MetadatumType.UNOFFICIAL_REFERENCE),
...
private fun extractDocumentStatus(norm: Norm): List<DocumentStatus> =
    norm.metadataSections
        ...
        .map { section ->
          ...
          DocumentStatus(
              section.metadata
                  .filter { it.type == MetadatumType.WORK_NOTE }
                  .map { it.value.toString() },
              section.metadata
                  .find { it.type == MetadatumType.DESCRIPTION }
                  ?.let { it.value.toString() },
              date ?: year,
              section.metadata
                  .find { it.type == MetadatumType.REFERENCE }
                  ?.let { it.value.toString() },
          )
        }
...
private fun extractDocumentTextProof(norm: Norm): String? {
  val metadata =
      norm.metadataSections
          .filter { it.name == Section.DOCUMENT_STATUS_SECTION }
          .flatMap { it.sections ?: listOf() }
          .find { it.name == Section.DOCUMENT_TEXT_PROOF }
          ?.metadata
...
private fun extractFootnotes(norm: Norm): List<Footnote> {
  fun transformFootnotes(section: MetadataSection, type: MetadatumType): List<Pair<Int, String>> {
    return section.metadata
        ...
        .map { metadata -> Pair(metadata.order, metadata.value.toString()) }
  }
  return norm.metadataSections
      .filter { section -> section.name == Section.FOOTNOTES }
      .map { footnoteSection ->
        val reference =
            footnoteSection.metadata
                .find { metadata -> metadata.type == MetadatumType.FOOTNOTE_REFERENCE }
                ?.let { it.value.toString() }
        Footnote(
            reference,
...
private fun extractCategorizedReferences(norm: Norm): List<CategorizedReference> =
    norm.metadataSections
...
private fun extractStatus(norm: Norm): List<Status> =
    norm.metadataSections
        ...
        .map { section ->
          Status(
              ...
              section.metadata
                  .find { it.type == MetadatumType.DATE }
                  ?.let { found -> encodeLocalDateToGermanFormat(found.value as LocalDate) },
              section.metadata
                  .filter { it.type == MetadatumType.REFERENCE }
                  .joinToString { it.value.toString() },
          )
        }

private fun extractReissue(norm: Norm): List<Reissue> =
    norm.metadataSections
        .filter { it.name == Section.STATUS_INDICATION }
        .flatMap { it.sections ?: listOf() }
...
private fun extractRepealStatus(norm: Norm): List<String> =
    norm.metadataSections
        .filter { it.name == Section.STATUS_INDICATION }
        .flatMap { it.sections ?: listOf() }
...
private fun extractOtherStatus(norm: Norm): List<String> =
    norm.metadataSections
        .filter { it.name == Section.STATUS_INDICATION }
        .flatMap { it.sections ?: listOf() }
...
private fun extractNormProviders(norm: Norm): List<NormProvider> =
    norm.metadataSections
        .filter { section -> section.name == Section.NORM_PROVIDER }
        .map {
          val entity =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.ENTITY }
                  ?.let { found -> found.value.toString() }
          val decidingBody =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.DECIDING_BODY }
                  ?.let { found -> found.value.toString() }
          val isResolutionMajority =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.RESOLUTION_MAJORITY }
                  ?.let { found -> found.value.toString().toBoolean() }
          NormProvider(entity, decidingBody, isResolutionMajority)
        }

private fun extractParticipations(norm: Norm): List<Participation> =
    norm.metadataSections
        .filter { section -> section.name == Section.PARTICIPATION }
        .map {
          val institution =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.PARTICIPATION_INSTITUTION }
                  ?.let { found -> found.value.toString() }
          val type =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.PARTICIPATION_TYPE }
                  ?.let { found -> found.value.toString() }
          Participation(type, institution)
        }

private fun extractLeads(norm: Norm): List<Lead> =
    norm.metadataSections
        .filter { section -> section.name == Section.LEAD }
        .map {
          val jurisdiction =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.LEAD_JURISDICTION }
                  ?.let { found -> found.value.toString() }
          val unit =
              it.metadata
                  .find { metadatum -> metadatum.type == MetadatumType.LEAD_UNIT }
                  ?.let { found -> found.value.toString() }
...
private fun extractDivergentEntryIntoForces(norm: Norm): List<DivergentEntryIntoForce> =
    norm.metadataSections
        ...
        .map {
          ...
          val normCategory =
              it.metadata
                  .firstOrNull { metadatum -> metadatum.type == MetadatumType.NORM_CATEGORY }
                  ?.let { found ->
                    parseDomainNormToData(NormCategory.valueOf(found.value.toString()))
                  }
          DivergentEntryIntoForce(date, state, normCategory)
        }

private fun extractDivergentExpirations(norm: Norm): List<DivergentExpiration> =
    norm.metadataSections
...
private fun extractDocumentType(norm: Norm): DocumentType? =
    norm.metadataSections
        ...
        .map { section ->
          DocumentType(
              ...
              section.metadata
                  .filter { it.type == MetadatumType.TEMPLATE_NAME }
                  .map { it.value.toString() },
              section.metadata
                  .filter { it.type == MetadatumType.NORM_CATEGORY }
                  .mapNotNull { parseDomainNormToData(NormCategory.valueOf(it.value.toString())) },
          )
        }
        .firstOrNull()

private fun extractPrintAnnouncementList(norm: Norm): List<PrintAnnouncement> =
    norm.metadataSections
        ...
        .map { section ->
          PrintAnnouncement(
              ...
              section.metadata
                  .find { it.type == MetadatumType.ANNOUNCEMENT_GAZETTE }
                  ?.value
                  .toString(),
          )
        }

private fun extractDigitalAnnouncementList(norm: Norm): List<DigitalAnnouncement> =
    norm.metadataSections
        .filter { it.name == Section.OFFICIAL_REFERENCE }
        .flatMap { it.sections ?: listOf() }
        ...
        .map { section ->
          DigitalAnnouncement(
              ...
              section.metadata
                  .find { it.type == MetadatumType.ANNOUNCEMENT_MEDIUM }
                  ?.value
                  .toString(),
          )
        }

private fun extractAgeIndicationStarts(norm: Norm): List<String> =
    norm.metadataSections
        .filter { section -> section.name == Section.AGE_INDICATION }
        .mapNotNull {
          it.metadata
              .find { metadatum -> metadatum.type == MetadatumType.RANGE_START }
              ?.let { found -> found.value.toString() }
        }

private fun extractEntryIntoForceDate(norm: Norm): String? =
    norm.metadataSections
...
private fun extractExpirationDate(norm: Norm): String? =
    norm.metadataSections
...
private fun parseDomainNormToData(normCategory: NormCategory?): String? =
    when (normCategory) {
