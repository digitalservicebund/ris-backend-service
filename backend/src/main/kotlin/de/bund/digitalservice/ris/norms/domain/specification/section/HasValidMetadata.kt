...
val hasValidMetadata =
    object : Specification<MetadataSection> {
        override fun isSatisfiedBy(instance: MetadataSection): Boolean =
            when (instance.name) {
                Section.OFFICIAL_REFERENCE,
                Section.DIVERGENT_ENTRY_INTO_FORCE,
                Section.DIVERGENT_EXPIRATION,
                Section.DOCUMENT_STATUS_SECTION,
                Section.STATUS_INDICATION,
                -> hasNone(instance)
                Section.CITATION_DATE, Section.PUBLICATION_DATE ->
                    hasOneOfType(listOf(DATE, YEAR), instance)
                Section.ENTRY_INTO_FORCE,
                Section.PRINCIPLE_ENTRY_INTO_FORCE,
                Section.EXPIRATION,
                Section.PRINCIPLE_EXPIRATION,
                ->
                    hasOneOfType(listOf(DATE, UNDEFINED_DATE), instance)
                Section.DIVERGENT_ENTRY_INTO_FORCE_DEFINED,
                Section.DIVERGENT_EXPIRATION_DEFINED,
                ->
                    hasType(listOf(DATE, NORM_CATEGORY), instance)
                Section.DIVERGENT_ENTRY_INTO_FORCE_UNDEFINED,
                Section.DIVERGENT_EXPIRATION_UNDEFINED,
                ->
                    hasType(listOf(UNDEFINED_DATE, NORM_CATEGORY), instance)
                Section.CATEGORIZED_REFERENCE, Section.REPEAL ->
                    hasType(listOf(TEXT), instance)
                Section.NORM ->
                    hasType(
                        listOf(
                            VALIDITY_RULE,
                ...
                Section.PARTICIPATION ->
                    hasType(
                        listOf(PARTICIPATION_TYPE, PARTICIPATION_INSTITUTION),
                        instance,
                    )
                ...
                Section.OTHER_OFFICIAL_ANNOUNCEMENT ->
                    hasType(listOf(OTHER_OFFICIAL_REFERENCE), instance)
                Section.NORM_PROVIDER ->
                    hasType(
                        listOf(ENTITY, DECIDING_BODY, RESOLUTION_MAJORITY),
                        instance,
                    )
                Section.DOCUMENT_TYPE ->
                    hasType(listOf(TYPE_NAME, NORM_CATEGORY, TEMPLATE_NAME), instance)
                Section.DIGITAL_EVIDENCE ->
                    hasType(
                        listOf(LINK, RELATED_DATA, EXTERNAL_DATA_NOTE, APPENDIX),
                        instance,
                    )
                Section.FOOTNOTES ->
                    hasType(
                ...
                Section.REISSUE ->
                    hasType(listOf(NOTE, ARTICLE, DATE, YEAR, REFERENCE), instance)
                Section.OTHER_STATUS -> hasType(listOf(NOTE), instance)
                Section.ANNOUNCEMENT_DATE -> hasValidAnnouncementDate(instance)
            }

        private fun hasNone(instance: MetadataSection): Boolean = instance.metadata.isEmpty()
        private fun hasType(types: List<MetadatumType>, instance: MetadataSection): Boolean =
            instance.metadata.all { it.type in types }
        private fun hasOneOfType(
            types: List<MetadatumType>,
            instance: MetadataSection,
        ): Boolean = instance.metadata.count() == 1 && hasType(types, instance)
