...
class GenerateNormFileService(
  ...
  inner class StreamDataBox(
      val file: ByteArray,
      var norm: Norm,
      val lastFileReference: FileReference
  )
  ...
  override fun generateNormFile(command: GenerateNormFileUseCase.Command): Mono<FileReference> {
    ...
    return getNormByGuidOutputPort
        .getNormByGuid(queryGetNorm)
  ...
  private fun generateNewZip(streamDataBox: StreamDataBox): Mono<StreamDataBox> {
    val commandGenerateNormFile =
        GenerateNormFileOutputPort.Command(streamDataBox.norm, streamDataBox.file)
    return generateNormFileOutputPort.generateNormFile(commandGenerateNormFile).map { generatedFile
      ->
      StreamDataBox(generatedFile, streamDataBox.norm, streamDataBox.lastFileReference)
    }
  }

  private fun saveFile(streamDataBox: StreamDataBox): Mono<StreamDataBox> {
    val commandSaveFile =
        SaveFileOutputPort.Command(
            streamDataBox.file,
            streamDataBox.lastFileReference.name,
            streamDataBox.file.size.toLong())
    return saveFileOutputPort.saveFile(commandSaveFile).map { streamDataBox }
  }
  ...
  private fun createNewFileReference(streamDataBox: StreamDataBox): Mono<FileReference> {
    val fileReference =
        FileReference(
            streamDataBox.lastFileReference.name,
            getHashFromContent(streamDataBox.file),
            guid = UUID.randomUUID())
    val commandSaveFileReference =
        SaveFileReferenceOutputPort.Command(fileReference, streamDataBox.norm)
    return saveFileReferenceOutputPort.saveFileReference(commandSaveFileReference).flatMap {
      Mono.just(fileReference)
    }
  }
}
