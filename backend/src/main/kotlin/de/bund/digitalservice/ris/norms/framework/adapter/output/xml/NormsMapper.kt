  ...
  return NormDto(
      ...
      documentTypeName =
          getMappedValue(
              ...
              norm
                  .getFirstMetadatum(MetadataSectionName.DOCUMENT_TYPE, MetadatumType.TYPE_NAME)
                  ?.value
                  .toString(),
          ),
      documentNormCategory =
          getMappedValue(
              Property.DOCUMENT_NORM_CATEGORY,
              norm
                  .getFirstMetadatum(MetadataSectionName.DOCUMENT_TYPE, MetadatumType.NORM_CATEGORY)
                  ?.value
                  .toString(),
          ),
      providerDecidingBody =
          getMappedValue(
      ...
      articles =
          norm.articles
              .filter { it.marker !in listOf("Eingangsformel", "Schlussformel") }
              .sortedBy {
...
fun mapParagraphToDto(
    paragraph: Paragraph,
    articleMarker: String,
    ordinalNumber: Int = 1
): ParagraphDto {
  val textNoUnknownTags =
      paragraph.text
          .replace("<Rec>", "")
          .replace("</Rec>", "")
          .replace("<Citation>", "")
          ...
          .replace(
              Regex("<FnR[ ]ID=\\\".*\"><\\/FnR>"),
              "") // DIN EN 15940<FnR ID="F816768_02"></FnR>, Ausgabe
  ...
  return ParagraphDto(
      ...
      content =
          if (textNoUnknownTags.contains("<DL")) toContentDto(textNoUnknownTags, paragraphMarker)
          else ContentDto(isText = true, text = IdentifiedElement(textNoUnknownTags)),
  )
}
...
fun parseChildren(node: Element, paragraphMarker: String, order: Int = 1): MutableList<ContentDto> {
  ...
  if (node.nodeName() === "dl") {
    ...
    for (childNumber in 0 until node.childrenSize() step 2) {
      ...
      childContentDtoList.addAll(
          parseChildren(ddElement.child(0), paragraphMarker, (childNumber / 2) + 1))
    }
  }
  if (node.nodeName() == "la") {
