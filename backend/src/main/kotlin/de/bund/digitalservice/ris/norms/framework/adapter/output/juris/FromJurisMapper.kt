...
fun mapDataToDomain(guid: UUID, data: NormData): Norm {
  val participationType =
      createMetadataForType(
          data.participationList.mapNotNull { it.type }, MetadatumType.PARTICIPATION_TYPE)
  val participationInstitution =
      createMetadataForType(
          data.participationList.mapNotNull { it.institution },
          MetadatumType.PARTICIPATION_INSTITUTION)

  val leadJurisdiction =
      createMetadataForType(
          data.leadList.mapNotNull { it.jurisdiction }, MetadatumType.LEAD_JURISDICTION)
  val leadUnit =
      createMetadataForType(data.leadList.mapNotNull { it.unit }, MetadatumType.LEAD_UNIT)

  val subjectFna =
      createMetadataForType(data.subjectAreaList.mapNotNull { it.fna }, MetadatumType.SUBJECT_FNA)
  val subjectGesta =
      createMetadataForType(
          data.subjectAreaList.mapNotNull { it.gesta }, MetadatumType.SUBJECT_GESTA)

  val citationDateSections =
      data.citationDateList.mapIndexed { index, value ->
        if (value.length == 4 && value.toIntOrNull() != null) {
          MetadataSection(
              Section.CITATION_DATE, listOf(Metadatum(value, MetadatumType.YEAR, 1)), index)
        } else if (value.length > 4) {
          val date = parseDateString(value)
          date?.let {
            MetadataSection(
                Section.CITATION_DATE, listOf(Metadatum(date, MetadatumType.DATE, 1)), index)
          }
        } else {
          null
        }
  ...
  val ageIndicationSections =
      data.ageIndicationStartList.mapIndexed { index, value ->
        MetadataSection(
            Section.AGE_INDICATION, listOf(Metadatum(value, MetadatumType.RANGE_START, 1)), index)
      }

  val categorizedReferenceSections =
      data.categorizedReferences
          .map { it.text }
          .filterNotNull()
          .mapIndexed { index, value ->
            MetadataSection(
                Section.CATEGORIZED_REFERENCE, listOf(Metadatum(value, MetadatumType.TEXT)), index)
          }

  val sections =
      listOf(createSectionForNorm(data)) +
          ...
          createSectionsFromMetadata(
              Section.PARTICIPATION, participationType + participationInstitution) +
          createSectionsForOfficialReference(
              data.digitalAnnouncementList, data.printAnnouncementList) +
...
fun createSectionsForDocumentStatus(
  ...
  documentStatus.forEachIndexed { index, documentStatusGroup ->
    ...
    if (documentStatusGroup.documentStatusWorkNote.isNotEmpty()) {
      metadata.add(
          Metadatum(documentStatusGroup.documentStatusWorkNote[0], MetadatumType.WORK_NOTE))
    }

    documentStatusGroup.documentStatusDescription?.let {
      metadata.add(Metadatum(it, MetadatumType.DESCRIPTION))
    }
    documentStatusGroup.documentStatusDateYear?.let {
      metadata.add(Metadatum(it, MetadatumType.YEAR))
    }
    documentStatusGroup.documentStatusReference?.let {
      metadata.add(Metadatum(it, MetadatumType.REFERENCE))
    }
...
private fun createSectionForDocumentType(documentType: DocumentType?): MetadataSection? {
  return if (documentType !== null) {
    val documentNormCategories =
        createMetadataForType(
            documentType.categories.mapNotNull { parseNormCategory(it) },
...
private fun createMetadatumForNullableString(
    string: String?,
    type: MetadatumType
): List<Metadatum<String>> = string?.let { listOf(Metadatum(string, type)) } ?: listOf()

private fun createSectionForNorm(data: NormData): MetadataSection {
  val divergentDocumentNumber =
      createMetadatumForNullableString(
          data.divergentDocumentNumber, MetadatumType.DIVERGENT_DOCUMENT_NUMBER)
  val frameKeywords = createMetadataForType(data.frameKeywordList, MetadatumType.KEYWORD)
  val risAbbreviationInternationalLaw =
      createMetadataForType(
          data.risAbbreviationInternationalLawList,
          MetadatumType.RIS_ABBREVIATION_INTERNATIONAL_LAW)
  val unofficialLongTitle =
      createMetadataForType(data.unofficialLongTitleList, MetadatumType.UNOFFICIAL_LONG_TITLE)
  val unofficialShortTitle =
      createMetadataForType(data.unofficialShortTitleList, MetadatumType.UNOFFICIAL_SHORT_TITLE)
  val unofficialAbbreviation =
      createMetadataForType(data.unofficialAbbreviationList, MetadatumType.UNOFFICIAL_ABBREVIATION)
  val unofficialReference =
      createMetadataForType(data.unofficialReferenceList, MetadatumType.UNOFFICIAL_REFERENCE)
  val referenceNumber =
      createMetadataForType(data.referenceNumberList, MetadatumType.REFERENCE_NUMBER)
  val definition = createMetadataForType(data.definitionList, MetadatumType.DEFINITION)
  val ageOfMajorityIndication =
      createMetadataForType(
          data.ageOfMajorityIndicationList, MetadatumType.AGE_OF_MAJORITY_INDICATION)
  val validityRule = createMetadataForType(data.validityRuleList, MetadatumType.VALIDITY_RULE)
  val officialLongTitle =
      createMetadatumForNullableString(data.officialLongTitle, MetadatumType.OFFICIAL_LONG_TITLE)
  val risAbbreviation =
      createMetadatumForNullableString(data.risAbbreviation, MetadatumType.RIS_ABBREVIATION)
  val documentCategory =
      createMetadatumForNullableString(data.documentCategory, MetadatumType.DOCUMENT_CATEGORY)
  val officialShortTitle =
      createMetadatumForNullableString(data.officialShortTitle, MetadatumType.OFFICIAL_SHORT_TITLE)
  val officialAbbreviation =
      createMetadatumForNullableString(
          data.officialAbbreviation, MetadatumType.OFFICIAL_ABBREVIATION)
...
private fun createSectionsForDivergentEntryIntoForce(
    data: List<DivergentEntryIntoForce>
): List<MetadataSection> {
  val definedDate =
      createMetadataForType(
          data
              ...
              .map { it.date }
              .filterNotNull()
              .map { parseDateString(it) },
          MetadatumType.DATE)
  val definedCategory =
      createMetadataForType(
          data.filter { it.state == null }.mapNotNull { parseNormCategory(it.normCategory) },
          MetadatumType.NORM_CATEGORY)
  val undefinedDate =
      createMetadataForType(
          data.mapNotNull { parseDateStateString(it.state) }, MetadatumType.UNDEFINED_DATE)
  val undefinedCategory =
      createMetadataForType(
          data.filter { it.state != null }.mapNotNull { parseNormCategory(it.normCategory) },
          MetadatumType.NORM_CATEGORY)
  ...
  val sections =
      createSectionsFromMetadata(
          Section.DIVERGENT_ENTRY_INTO_FORCE_DEFINED, definedDate + definedCategory) +
          createSectionsFromMetadata(
              Section.DIVERGENT_ENTRY_INTO_FORCE_UNDEFINED, undefinedDate + undefinedCategory)

  return sections.mapIndexed { index, section ->
    MetadataSection(Section.DIVERGENT_ENTRY_INTO_FORCE, listOf(), index, listOf(section))
  }
}

private fun createSectionsForDivergentExpiration(
    data: List<DivergentExpiration>
): List<MetadataSection> {
  val definedDate =
      createMetadataForType(
          data
  ...
  val sections =
      createSectionsFromMetadata(
          Section.DIVERGENT_EXPIRATION_DEFINED, definedDate + definedCategory) +
          createSectionsFromMetadata(
              Section.DIVERGENT_EXPIRATION_UNDEFINED, undefinedDate + undefinedCategory)

  return sections.mapIndexed { index, section ->
    MetadataSection(Section.DIVERGENT_EXPIRATION, listOf(), index, listOf(section))
  }
}
...
private fun createSectionsForOfficialReference(
    digitalAnnouncement: List<DigitalAnnouncement>,
    printAnnouncement: List<PrintAnnouncement>
): List<MetadataSection> {
  val printAnnouncementGazette =
      createMetadataForType(
          printAnnouncement.mapNotNull { it.gazette }, MetadatumType.ANNOUNCEMENT_GAZETTE)
  val printAnnouncementPage =
      createMetadataForType(printAnnouncement.mapNotNull { it.page }, MetadatumType.PAGE)
  val printAnnouncementYear =
      createMetadataForType(printAnnouncement.mapNotNull { it.year }, MetadatumType.YEAR)
  val digitalAnnouncementYear =
      createMetadataForType(digitalAnnouncement.mapNotNull { it.year }, MetadatumType.YEAR)
  val digitalAnnouncementNumber =
      createMetadataForType(digitalAnnouncement.mapNotNull { it.number }, MetadatumType.EDITION)
  val digitalAnnouncementMedium =
      createMetadataForType(
          digitalAnnouncement.mapNotNull { it.medium }, MetadatumType.ANNOUNCEMENT_MEDIUM)

  val referenceSections =
      createSectionsFromMetadata(
          Section.PRINT_ANNOUNCEMENT,
          printAnnouncementGazette + printAnnouncementYear + printAnnouncementPage) +
          createSectionsFromMetadata(
              Section.DIGITAL_ANNOUNCEMENT,
              digitalAnnouncementNumber + digitalAnnouncementMedium + digitalAnnouncementYear)

  return referenceSections.mapIndexed { index, section ->
    MetadataSection(Section.OFFICIAL_REFERENCE, listOf(), index, listOf(section))
  }
}
...
private fun createFootnoteSections(footnotes: List<Footnote>): List<MetadataSection> {
  ...
  footnotes.forEachIndexed { index, footnoteGroup ->
    fun addFootnotesToMetadata(
        footnoteList: List<Pair<Int, String>>,
        type: MetadatumType,
        metadata: MutableList<Metadatum<*>>,
        addToFootnoteOrder: Int
    ) {
      footnoteList.forEach { singleFootnote ->
        metadata.add(
            Metadatum(singleFootnote.second, type, singleFootnote.first + addToFootnoteOrder))
      }
    }

    val metadata = mutableListOf<Metadatum<*>>()

    footnoteGroup.reference?.let {
      metadata.add(Metadatum(it, MetadatumType.FOOTNOTE_REFERENCE, 1))
    }

    val addToFootnotesOrder = if (footnoteGroup.reference != null) 1 else 0
    addFootnotesToMetadata(
        footnoteGroup.footnoteChange, MetadatumType.FOOTNOTE_CHANGE, metadata, addToFootnotesOrder)
    addFootnotesToMetadata(
        footnoteGroup.footnoteComment,
        MetadatumType.FOOTNOTE_COMMENT,
        metadata,
        addToFootnotesOrder)
    addFootnotesToMetadata(
        footnoteGroup.footnoteDecision,
        MetadatumType.FOOTNOTE_DECISION,
        metadata,
        addToFootnotesOrder)
    addFootnotesToMetadata(
        footnoteGroup.footnoteStateLaw,
        MetadatumType.FOOTNOTE_STATE_LAW,
        metadata,
        addToFootnotesOrder)
    addFootnotesToMetadata(
        footnoteGroup.footnoteEuLaw, MetadatumType.FOOTNOTE_EU_LAW, metadata, addToFootnotesOrder)
    addFootnotesToMetadata(
        footnoteGroup.otherFootnote, MetadatumType.FOOTNOTE_OTHER, metadata, addToFootnotesOrder)

    if (metadata.isNotEmpty())
        footnoteSections.add(MetadataSection(Section.FOOTNOTES, metadata, index + 1))
  }

  return footnoteSections
}

private fun createSectionForStatusIndication(
    statusList: List<Status>,
    reissueList: List<Reissue>,
    repealList: List<String>,
    otherStatusList: List<String>
): List<MetadataSection> {
  ...
  statusSections.addAll(
      otherStatusList.mapIndexed { index, otherStatus ->
        MetadataSection(
            Section.OTHER_STATUS, listOf(Metadatum(otherStatus, MetadatumType.NOTE)), index + 1)
      },
  )
...
fun addProviderSections(normProviders: List<NormProvider>): List<MetadataSection> {
  return normProviders
      .mapIndexed { index, normProvider ->
        val metadata = mutableListOf<Metadatum<*>>()

        normProvider.entity?.let { metadata.add(Metadatum(it, MetadatumType.ENTITY, 1)) }

        normProvider.decidingBody?.let {
          metadata.add(Metadatum(it, MetadatumType.DECIDING_BODY, 1))
        }
        normProvider.isResolutionMajority?.let {
          metadata.add(Metadatum(it, MetadatumType.RESOLUTION_MAJORITY, 1))
        }

        if (metadata.isNotEmpty()) MetadataSection(Section.NORM_PROVIDER, metadata, index + 1)
        else null
      }
      .filterNotNull()
}
...
fun parseNormCategory(value: String?): NormCategory? =
    when (value) {
...
fun createSectionsFromMetadata(sectionName: MetadataSectionName, metadata: List<Metadatum<*>>) =
    metadata
        .groupBy { it.order }
        .mapValues {
          MetadataSection(
...
private fun createSectionsWithoutGrouping(
    sectionName: MetadataSectionName,
    metadata: List<Metadatum<*>>
) =
    metadata.mapIndexed { index, metadatum ->
