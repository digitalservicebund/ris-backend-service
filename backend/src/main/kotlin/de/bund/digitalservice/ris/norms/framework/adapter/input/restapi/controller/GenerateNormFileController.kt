...
class GenerateNormFileController(private val generateNormFileService: GenerateNormFileUseCase) {
  ...
  @Operation(
      ...
      description =
          "Generate a new compressed ZIP file containing all juris xml files for the latest edits of a norm")
  @ApiResponses(
      ApiResponse(responseCode = "201", description = "Zip File Created"),
      ApiResponse(responseCode = "400"),
  )
  fun generateNormFile(
      @PathVariable guid: String
  ): Mono<ResponseEntity<FileReferenceResponseSchema>> {
    val command = GenerateNormFileUseCase.Command(UUID.fromString(guid))
    return generateNormFileService
        .generateNormFile(command)
        .map { fileReference -> FileReferenceResponseSchema.fromUseCaseData(fileReference) }
        .map { fileReferenceResponseSchema ->
          ResponseEntity.ok().body(fileReferenceResponseSchema)
        }
        .switchIfEmpty(Mono.error(NotFoundWithInstanceException(URI.create(guid))))
  }
