...
val legalDocMlValuesMap: List<MappedValues> =
    listOf(
        MappedValues(
...
fun getMappedValue(property: Property, value: String): String? {
  val mappedValues = legalDocMlValuesMap.firstOrNull { it.property === property }

  return mappedValues
      ?.values
      ?.firstOrNull { it.oldValues.any { oldValue -> value.contains(Regex("^$oldValue$")) } }
      ?.newValue
