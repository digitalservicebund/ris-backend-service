...
class NormsServiceTest : PostgresTestcontainerIntegrationTest() {
  ...
  @Autowired private lateinit var normsService: NormsService
  ...
  fun `the search norm result includes their ELI property`() {
    ...
    val officialReference =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, emptyList(), 1, listOf(printAnnouncement))
    ...
    normsService
        .searchNorms(searchQuery)
        .`as`(StepVerifier::create)
        .assertNext {
          assertThat(it.eli.toString()).isNotEqualTo("")
          assertThat(
                  it.metadataSections
                      .first { it.name == MetadataSectionName.NORM }
                      .metadata
                      .first { it.type == OFFICIAL_LONG_TITLE })
              .isEqualTo(officialLongTitle)
        }
        .verifyComplete()
  }
  ...
  fun `save simple norm and retrieved by eli`() {
    ...
    val referenceSection1 =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 1, listOf(printAnnouncementSection))
  ...
  fun `save multiple norms and retrieve only one by eli parameters`() {
    ...
    val referenceSection2 =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 1, listOf(printAnnouncementSection2))
    val secondNorm =
        Norm(
            guid = UUID.randomUUID(),
    ...
    normsService
        .saveNorm(saveFirstNormCommand)
        .`as`(StepVerifier::create)
        .expectNextCount(1)
        .verifyComplete()

    normsService
        .saveNorm(saveSecondNormCommand)
        .`as`(StepVerifier::create)
        .expectNextCount(1)
        .verifyComplete()
        ...
        Norm(
            ...
            metadataSections =
                listOf(
                    citationDateSection,
                    referenceSection1,
                    createAnnouncementSection("2022-02-02")),
        )
    val saveCommand = SaveNormOutputPort.Command(norm)
    val eliQuery = GetNormByEliOutputPort.Query("bg-1", "2001", "1125")
  ...
  fun `save a norm with official reference sections and retrieve it by guid`() {
    ...
    val digitalAnnouncementSection =
        MetadataSection(
    ...
    val referenceSection2 =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 2, listOf(digitalAnnouncementSection))
    val norm =
        Norm(
            guid = UUID.randomUUID(),
  ...
  fun `save norm with 2 sections that are having the same types and get them back`() {
    ...
    normsService
        ...
        .`as`(StepVerifier::create)
        .assertNext {
          assertThat(it.metadataSections.groupBy { it.order })
              .isEqualTo(norm.metadataSections.groupBy { it.order })
        }
        .verifyComplete()
  }
  ...
  fun `save norm and edit norm`() {
    ...
    normsService
        ...
        .assertNext {
          ...
          assertThat(metadata.first { it.type == COMPLETE_CITATION }.value)
              .isEqualTo("complete citation")
          assertThat(metadata.first { it.type == DOCUMENT_NUMBER }.value)
  ...
  fun `save a file reference to a norm and retrieve norm by guid`() {
    ...
    normsService
        .saveNorm(saveNormCommand)
        .`as`(StepVerifier::create)
        .expectNextCount(1)
        .verifyComplete()

    normsService
        .saveFileReference(saveFileReferenceCommand)
        .`as`(StepVerifier::create)
        .expectNextCount(1)
        .verifyComplete()
  ...
  fun `save a norm with no print announcement and retrieve by eli with digital announcement and edition`() {
    ...
    val referenceSection =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 1, listOf(digitalAnnouncement))
    val norm =
        Norm(
            guid = UUID.randomUUID(),
  ...
  fun `save a norm with print announcement and digital announcement and dont retrieve by eli because only digital announcement matches`() {
    ...
    val referenceSectionPrint =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 1, listOf(printAnnouncement))
    val referenceSectionDigital =
        MetadataSection(
            MetadataSectionName.OFFICIAL_REFERENCE, listOf(), 2, listOf(digitalAnnouncement))
    val norm =
        Norm(
            guid = UUID.randomUUID(),
            metadataSections =
...
private fun assertNormsAreEqual(norm1: Norm, norm2: Norm) =
    assertThat(norm1)
...
/**
 * We need to reduce to milliseconds (don't care about micro or nano seconds) since the PostgreSQL
 * database can only save up to 6 digits and automatically rounds up.
 */
private fun localDatesAreEqualUpToMilliseconds(
    first: LocalDateTime,
