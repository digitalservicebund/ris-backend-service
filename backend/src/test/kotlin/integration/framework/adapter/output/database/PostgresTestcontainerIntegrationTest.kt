 ...
 * testcontainer. All test classes inheriting from this class will automatically use the same
 * database container instance.
 *
 * Instead of letting testcontainers start the database container automatically just by
 * configuration, it is created manually here due to the following reasons:
 * - All integration tests shall reuse the same database instance to speed up the tests
 * - Flyway does not support R2DBC and therefore uses JDBC to access the database which results in
 *   the Flyway migrations being executed on a different instance than the tests when using
 *   automatic start of containers.
 *
 * @see <a href="https://github.com/testcontainers/testcontainers-java/issues/4473">Testcontainers
 *   GitHub Issue</a>
 */
@Tag("integration")
open class PostgresTestcontainerIntegrationTest {
  companion object {
    @Container
    val postgresql =
        PostgresTestcontainer.instance!!.apply {
