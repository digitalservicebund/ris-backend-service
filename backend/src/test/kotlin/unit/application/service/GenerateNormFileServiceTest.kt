...
class GenerateNormFileServiceTest {
  ...
  fun `it calls the get norm by GUID adapter using the input query GUID parameter`() {
    ...
    val service =
        GenerateNormFileService(
            getNormByGuidOutputPort,
            getFileOutputPort,
            generateNormFileOutputPort,
            saveFileOutputPort,
    ...
    verify(exactly = 1) {
      getNormByGuidOutputPort.getNormByGuid(withArg { assertThat(it.guid).isEqualTo(guid) })
    }
  }
  ...
  fun `it calls the get file by hash adapter using the found norm`() {
    ...
    verify(exactly = 1) {
      getFileOutputPort.getFile(withArg { assertThat(it.hash).isEqualTo(fileReference.hash) })
    }
  }
