...
class JurisConverterTest {
  ...
  inner class ParseJurisXml {
    ...
    fun `it correctly maps the parsed data to the norm properties`() {
      ...
      val data =
          NormData().apply {
            ...
            participationList =
                listOf(Participation("test participation type", "test participation institution"))
            ...
            divergentEntryIntoForceList =
                listOf(
                    DivergentEntryIntoForce("2022-01-03", null, "SN"),
                    DivergentEntryIntoForce(null, "UNDEFINED_NOT_PRESENT", "ÜN"))
            expirationDate = null
            ...
            divergentExpirationsList =
                listOf(
                    DivergentExpiration("2022-01-06", null, "ÄN"),
                    DivergentExpiration(null, "UNDEFINED_UNKNOWN", "SN"))
            announcementDate = "2022-01-07"
            citationDateList = listOf("2022-01-08")
            printAnnouncementList =
                listOf(
                    PrintAnnouncement(
                        "test print announcement year",
                        "test print announcement page",
                        "test print announcement gazette"))
            ...
            categorizedReferences =
                listOf(
                    CategorizedReference("test categorized reference 1"),
                    CategorizedReference("test categorized reference 2"))
          }
      val query = ParseJurisXmlOutputPort.Query(anyGuid, anyZipFile.readBytes(), anyZipFile.name)
      every { extractData(any()) } returns data
      ...
      assertThat(metadata)
          ...
          .contains(Metadatum("test official long title", OFFICIAL_LONG_TITLE))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test ris abbreviation", RIS_ABBREVIATION))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test document category", DOCUMENT_CATEGORY))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test official short title", OFFICIAL_SHORT_TITLE))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test official abbreviation", OFFICIAL_ABBREVIATION))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test celex number", CELEX_NUMBER))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test text", TEXT))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test document number", DIVERGENT_DOCUMENT_NUMBER, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(
              Metadatum(
                  "test ris abbreviation international law", RIS_ABBREVIATION_INTERNATIONAL_LAW, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test unofficial long title", UNOFFICIAL_LONG_TITLE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test unofficial short title", UNOFFICIAL_SHORT_TITLE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test unofficial abbreviation", UNOFFICIAL_ABBREVIATION, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test unofficial reference", UNOFFICIAL_REFERENCE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test validity rule", VALIDITY_RULE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test reference number", REFERENCE_NUMBER, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test definition", DEFINITION, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test age of majority indication", AGE_OF_MAJORITY_INDICATION, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test participation type", PARTICIPATION_TYPE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test participation institution", PARTICIPATION_INSTITUTION, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test lead jurisdiction", LEAD_JURISDICTION, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test lead unit", LEAD_UNIT, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test subject FNA", SUBJECT_FNA, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test subject Gesta", SUBJECT_GESTA, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-08"), DATE, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test provider entity", ENTITY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test provider deciding body", DECIDING_BODY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(true, RESOLUTION_MAJORITY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("DEU", ENTITY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("BT", DECIDING_BODY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(false, RESOLUTION_MAJORITY, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("Lebensjahr 10", RANGE_START, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("Monate 11", RANGE_START, 1))

      val printAnnouncementMetadata =
          norm?.metadataSections?.flatMap { it.sections ?: listOf() }?.flatMap { it.metadata }
      assertThat(printAnnouncementMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test print announcement gazette", ANNOUNCEMENT_GAZETTE, 1))
      assertThat(printAnnouncementMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test print announcement year", YEAR, 1))
      assertThat(printAnnouncementMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test print announcement page", PAGE, 1))

      val documentTypeSections =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.DOCUMENT_TYPE }
      assertThat(documentTypeSections).hasSize(1)
      val documentTypeMetadata = documentTypeSections?.flatMap { it.metadata }
      assertThat(documentTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("RV", TYPE_NAME, 1))
      assertThat(documentTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(NormCategory.TRANSITIONAL_NORM, NORM_CATEGORY, 1))
      assertThat(documentTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("documentTemplateName", TEMPLATE_NAME, 1))

      val divergentEntryIntoForceParentSections =
          norm?.metadataSections?.filter {
            it.name == MetadataSectionName.DIVERGENT_ENTRY_INTO_FORCE
          }
      assertThat(divergentEntryIntoForceParentSections).hasSize(2)
      val divergentEntryIntoForceChildrenSections =
          divergentEntryIntoForceParentSections?.mapNotNull { it.sections }?.flatten()
      assertThat(divergentEntryIntoForceChildrenSections).hasSize(2)
      val divergentEntryIntoForceDefinedMetadata =
          divergentEntryIntoForceChildrenSections
              ?.filter { it.name == MetadataSectionName.DIVERGENT_ENTRY_INTO_FORCE_DEFINED }
              ?.flatMap { it.metadata }
      assertThat(divergentEntryIntoForceDefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-03"), DATE, 1))
      assertThat(divergentEntryIntoForceDefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(NormCategory.BASE_NORM, NORM_CATEGORY, 1))
      val divergentEntryIntoForceUndefinedMetadata =
          divergentEntryIntoForceChildrenSections
              ?.filter { it.name == MetadataSectionName.DIVERGENT_ENTRY_INTO_FORCE_UNDEFINED }
              ?.flatMap { it.metadata }
      assertThat(divergentEntryIntoForceUndefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(UndefinedDate.UNDEFINED_NOT_PRESENT, UNDEFINED_DATE, 1))
      assertThat(divergentEntryIntoForceUndefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(NormCategory.TRANSITIONAL_NORM, NORM_CATEGORY, 1))

      val divergentexpirationParentSections =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.DIVERGENT_EXPIRATION }
      assertThat(divergentexpirationParentSections).hasSize(2)
      val divergentexpirationChildrenSections =
          divergentexpirationParentSections?.mapNotNull { it.sections }?.flatten()
      assertThat(divergentexpirationChildrenSections).hasSize(2)
      val divergentexpirationDefinedMetadata =
          divergentexpirationChildrenSections
              ?.filter { it.name == MetadataSectionName.DIVERGENT_EXPIRATION_DEFINED }
              ?.flatMap { it.metadata }
      assertThat(divergentexpirationDefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-06"), DATE, 1))
      assertThat(divergentexpirationDefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(NormCategory.AMENDMENT_NORM, NORM_CATEGORY, 1))
      val divergentexpirationUndefinedMetadata =
          divergentexpirationChildrenSections
              ?.filter { it.name == MetadataSectionName.DIVERGENT_EXPIRATION_UNDEFINED }
              ?.flatMap { it.metadata }
      assertThat(divergentexpirationUndefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(UndefinedDate.UNDEFINED_UNKNOWN, UNDEFINED_DATE, 1))
      assertThat(divergentexpirationUndefinedMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(NormCategory.BASE_NORM, NORM_CATEGORY, 1))

      val categorizedReferenceSections =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.CATEGORIZED_REFERENCE }
      assertThat(categorizedReferenceSections).hasSize(2)
      assertThat(categorizedReferenceSections?.get(0)?.metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test categorized reference 1", TEXT, 1))
      assertThat(categorizedReferenceSections?.get(1)?.metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test categorized reference 2", TEXT, 1))

      val entryIntoForceSection =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.ENTRY_INTO_FORCE }
      ...
      assertThat(entryIntoForceMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse(data.entryIntoForceDate), DATE, 1))

      val principleEntryIntoForceSection =
          norm?.metadataSections?.filter {
            it.name == MetadataSectionName.PRINCIPLE_ENTRY_INTO_FORCE
          }
      ...
      assertThat(principleEntryIntoForceMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(UndefinedDate.UNDEFINED_FUTURE, UNDEFINED_DATE, 1))

      val expirationSection =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.EXPIRATION }
      ...
      assertThat(expirationMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(UndefinedDate.UNDEFINED_UNKNOWN, UNDEFINED_DATE, 1))

      val principleExpirationSection =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.PRINCIPLE_EXPIRATION }
      ...
      assertThat(principleExpirationMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse(data.principleExpirationDate), DATE, 1))

      val statusIndications =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.STATUS_INDICATION }
      assertThat(statusIndications).hasSize(5)
      val statusIndicationsChildren = statusIndications?.mapNotNull { it.sections }?.flatten()
      val statusTypeSections =
          statusIndicationsChildren?.filter { it.name == MetadataSectionName.STATUS }
      assertThat(statusTypeSections).hasSize(1)
      val statusTypeMetadata = statusTypeSections?.get(0)?.metadata
      assertThat(statusTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test status note", NOTE))
      assertThat(statusTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test status description", DESCRIPTION))
      assertThat(statusTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-09"), DATE))
      assertThat(statusTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test status reference", REFERENCE))

      val reissueTypeSections =
          statusIndicationsChildren?.filter { it.name == MetadataSectionName.REISSUE }
      assertThat(reissueTypeSections).hasSize(1)
      val reissueTypeMetadata = reissueTypeSections?.get(0)?.metadata
      assertThat(reissueTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test reissue note", NOTE))
      assertThat(reissueTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test reissue article", ARTICLE))
      assertThat(reissueTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-11"), DATE))
      assertThat(reissueTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test reissue reference", REFERENCE))

      val repealTypeSections =
          statusIndicationsChildren?.filter { it.name == MetadataSectionName.REPEAL }
      assertThat(repealTypeSections).hasSize(2)
      val repealTypeMetadata = repealTypeSections?.map { it.metadata }?.flatten()
      assertThat(repealTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test repeal references 1", TEXT))
      assertThat(repealTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test repeal references 2", TEXT))

      val otherTypeSections =
          statusIndicationsChildren?.filter { it.name == MetadataSectionName.OTHER_STATUS }
      assertThat(otherTypeSections).hasSize(1)
      val otherTypeMetadata = otherTypeSections?.get(0)?.metadata
      assertThat(otherTypeMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test other status note", NOTE))

      val documentStatusParentSections =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.DOCUMENT_STATUS_SECTION }
      assertThat(documentStatusParentSections).hasSize(1)
      val documentStatusChildren =
          documentStatusParentSections?.mapNotNull { it.sections }?.flatten()
      val documentStatusSections =
          documentStatusChildren?.filter { it.name == MetadataSectionName.DOCUMENT_STATUS }
      assertThat(documentStatusSections).hasSize(1)
      val documentStatusMetadata = documentStatusSections?.get(0)?.metadata
      assertThat(documentStatusMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test document status work note", WORK_NOTE))
      assertThat(documentStatusMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("test document status description", DESCRIPTION))
      assertThat(documentStatusMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("2022", YEAR))

      val announcementDateSections =
          norm?.metadataSections?.filter { it.name == MetadataSectionName.ANNOUNCEMENT_DATE }
      assertThat(announcementDateSections).hasSize(1)
      val announcementDateMetadata = announcementDateSections?.get(0)?.metadata
      assertThat(announcementDateMetadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum(LocalDate.parse("2022-01-07"), DATE))
    }
    ...
    fun `it correctly maps the parsed data to metdata using the index order`() {
      ...
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("foo", KEYWORD, 1))
      assertThat(metadata)
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("bar", KEYWORD, 2))
    }
    ...
    fun `it correctly maps the citation date containing only year to the norm properties`() {
      ...
      assertThat(
              norm
                  ?.metadataSections
                  ?.flatMap { it.metadata }
                  ?.first { it.type == OFFICIAL_LONG_TITLE }
                  ?.value
                  .toString())
          .isEqualTo("test official long title")
      assertThat(norm?.metadataSections?.flatMap { it.metadata })
          .usingRecursiveFieldByFieldElementComparatorIgnoringFields("guid")
          .contains(Metadatum("2022", YEAR, 1))
      assertThat(norm?.metadataSections?.flatMap { it.metadata }?.filter { it.type == DATE })
          .isEmpty()
    }
    ...
    fun `it returns null citation date and year if date value invalid`() {
      ...
      assertThat(norm?.metadataSections?.flatMap { it.metadata }?.filter { it.type == DATE })
          .isEmpty()
      assertThat(norm?.metadataSections?.flatMap { it.metadata }?.filter { it.type == YEAR })
          .isEmpty()
      assertThat(
              norm
                  ?.metadataSections
  ...
  inner class GenerateNormFile {
        ...
        createRandomNorm()
            .copy(
                metadataSections =
                    listOf(
                        MetadataSection(
                            MetadataSectionName.CITATION_DATE, listOf(Metadatum("2002", YEAR))),
                    ),
            )
    ...
    fun `it correctly maps the norm metadata values including the order`() {
      ...
      verify(exactly = 1) {
        generateZip(
            withArg { assertThat(it.frameKeywordList).isEqualTo(listOf("foo", "bar")) },
            any(),
        )
      }
    }
