...
class LoadNormAsXmlControllerTest {
  ...
  fun `it calls the load service with the correct query to get the XML`() {
    ...
    webClient.mutateWith(csrf()).get().uri("/api/v1/norms/xml/eli/bg-1/2022/s1125").exchange()

    verify(exactly = 1) {
      loadNormAsXmlService.loadNormAsXml(
          withArg {
            assertThat(it.printAnnouncementGazette).isEqualTo("bg-1")
            assertThat(it.announcementOrCitationYear).isEqualTo("2022")
            assertThat(it.printAnnouncementPage).isEqualTo("1125")
          },
      )
    }
  }
