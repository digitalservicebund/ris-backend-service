...
fun assertEditNormFramePropertiesAndEditNormRequestSchema(
  ...
  normFramePropertiesMembers.forEach { normFramePropertiesMember ->
    if (normFramePropertiesMember.name == "metadataSections") {
      assertMetadataSections(
          normFrameProperties.metadataSections.filter {
            it.name != MetadataSectionName.DOCUMENT_TYPE
          },
          normFrameRequestSchema.metadataSections)
...
private fun assertMetadataSections(
    propertiesSections: List<MetadataSection>,
    schemaSections: List<*>
) {
  val guidRegex =
      Regex(
          "guid=[0-9a-fA-F]{8}\\b-[0-9a-fA-F]{4}\\b-[0-9a-fA-F]{4}\\b-[0-9a-fA-F]{4}\\b-[0-9a-fA-F]{12}")
