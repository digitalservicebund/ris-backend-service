...
import de.bund.digitalservice.ris.caselaw.adapter.publication.adm.RevokedAdmSyncJob;
...
class RevokedAdmSyncJobIntegrationTest extends BaseIntegrationTest {
  ...
  @Autowired private RevokedAdmSyncJob syncJob;
  ...
  void testRun() {
    ...
    jobSyncStatusRepository.save(
        new JobSyncStatus(SyncJob.ADM_REVOKED_SYNC.getName(), Instant.now().minusSeconds(300)));
    ...
    decision.setPassiveAdmCitations(
        List.of(
            PassiveCitationAdmDTO.builder()
                .sourceId(UUID.fromString("41519069-ae24-4dd9-a412-32f2d594fb86"))
    ...
    var statusAfterSync = jobSyncStatusRepository.findById(SyncJob.ADM_REVOKED_SYNC.getName());
    ...
    DecisionDTO updatedDecision =
        (DecisionDTO)
            databaseDocumentationUnitRepository
                .findById(UUID.fromString("adb8408b-5a77-48f9-9ed0-b8dee4f2db02"))
                .orElseThrow();
    assertThat(updatedDecision.getPassiveAdmCitations()).isEmpty();
    TestTransaction.end();
