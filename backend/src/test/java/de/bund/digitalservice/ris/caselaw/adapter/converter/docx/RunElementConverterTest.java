...
class RunElementConverterTest {
  ...
  @Test
  void testConvert_withVMLShapeType_logAWarning() {
    // given
    R rElement = new R();
    Pict pict = new Pict();
    CTShapetype shapetype = new CTShapetype();
    pict.getAnyAndAny().add(new JAXBElement<>(QName.valueOf(""), Object.class, shapetype));
    rElement.getContent().add(new JAXBElement<>(QName.valueOf(""), Pict.class, pict));

    ParagraphElement paragraphElement = new ParagraphElement();
    TestMemoryAppender memoryAppender = new TestMemoryAppender(RunElementConverter.class);

    // when
    RunElementConverter.convert(
        rElement, paragraphElement, mock(DocxConverter.class), new ArrayList<>());

    // then
    assertThat(memoryAppender.count(Level.WARN)).isEqualTo(1L);
    assertThat(memoryAppender.getMessage(Level.WARN, 0))
        .isEqualTo("Ignore shape type information.");
