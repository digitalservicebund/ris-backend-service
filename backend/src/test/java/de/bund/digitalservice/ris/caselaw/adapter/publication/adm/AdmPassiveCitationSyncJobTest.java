...
@Import({AdmPassiveCitationSyncJob.class})
class AdmPassiveCitationSyncJobTest {
  ...
  void shouldRunWithLastRunDateAndUpdateLastRun() {
    ...
    when(jobSyncStatusService.getLastRun(SyncJob.ADM_PASSIVE_CITATION_SYNC)).thenReturn(lastRun);

    admPassiveCitationSyncJob.runSync();

    verify(admCitationSyncService).handleNewlyPublishedAfter(lastRun);
    verify(jobSyncStatusService)
        .updateLastRun(
            eq(SyncJob.ADM_PASSIVE_CITATION_SYNC),
            assertArg(
                newLastRun -> {
