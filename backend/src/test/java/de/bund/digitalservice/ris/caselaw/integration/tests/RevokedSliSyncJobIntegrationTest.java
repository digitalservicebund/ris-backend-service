...
import de.bund.digitalservice.ris.caselaw.adapter.publication.sli.RevokedSliSyncJob;
...
class RevokedSliSyncJobIntegrationTest extends BaseIntegrationTest {
  ...
  @Autowired private RevokedSliSyncJob syncJob;
  ...
  @Sql(scripts = "classpath:sli_revoked_init.sql")
  @Transactional
  void testRun() {
    TestTransaction.end();
    jobSyncStatusRepository.save(
        new JobSyncStatus(SyncJob.SLI_REVOKED_SYNC.getName(), Instant.now().minusSeconds(300)));
    ...
    decision.setPassiveSliCitations(
        List.of(
            PassiveCitationSliEntity.builder()
                .sourceId(UUID.fromString("7df9d7fd-8ac5-4611-ad78-5a81660c0611"))
                .sourceDocumentNumber("KSNR004051609")
                .sourceAuthor("Beitel, Willibald")
    ...
    var statusAfterSync = jobSyncStatusRepository.findById(SyncJob.SLI_REVOKED_SYNC.getName());
    ...
    DecisionDTO updatedDecision =
        (DecisionDTO)
            databaseDocumentationUnitRepository
                .findById(UUID.fromString("adb8408b-5a77-48f9-9ed0-b8dee4f2db02"))
                .orElseThrow();
    assertThat(updatedDecision.getPassiveSliCitations()).isEmpty();
    TestTransaction.end();
