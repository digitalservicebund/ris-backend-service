...
@Import({RevokedSliSyncJob.class})
class RevokedSliSyncJobTest {
  @Autowired RevokedSliSyncJob revokedSliSyncJob;
  ...
  void shouldRunWithLastRunDateAndUpdateLastRun() {
    ...
    when(jobSyncStatusService.getLastRun(SyncJob.SLI_REVOKED_SYNC)).thenReturn(lastRun);

    revokedSliSyncJob.runSync();

    verify(sliCitationSyncService).handleRevokedAfter(lastRun);
    verify(jobSyncStatusService)
        .updateLastRun(
            eq(SyncJob.SLI_REVOKED_SYNC),
            assertArg(
                newLastRun -> {
