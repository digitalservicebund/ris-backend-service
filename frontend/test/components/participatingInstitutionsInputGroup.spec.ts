...
import ParticipatingInstitutionsInputGroup from "@/components/ParticipatingInstitutionsInputGroup.vue"

function renderComponent(options?: { modelValue?: unknown }) {
  const props = {
    modelValue: options?.modelValue,
  }
  const utils = render(ParticipatingInstitutionsInputGroup, { props })
  const user = userEvent.setup()
  return { user, ...utils }
}

async function getInputFields() {
  const input1 = (await screen.findByLabelText(
    "Art der Mitwirkung"
  )) as HTMLInputElement
...
describe("ParticipatingInstitutionsInputGroup", () => {
  it("renders an InputGroup with the given input fields", async () => {
    renderComponent({
      modelValue: { participationType: "", participationInstitution: "" },
    })
  ...
  it("renders input field with given data ", async () => {
    renderComponent({
      modelValue: { participationType: "foo", participationInstitution: "baz" },
    })
  ...
  it("should emit update:modelValue when input values with the new input values", async () => {
    ...
    expect(emitted()["update:modelValue"]).toEqual([
      [{ participationType: "foobar", participationInstitution: "bar" }],
      [{ participationType: "foobar", participationInstitution: "bar" }],
      [{ participationType: "foobar", participationInstitution: "bar" }],
    ])
  })
