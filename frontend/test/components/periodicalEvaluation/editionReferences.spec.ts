...
describe("Legal periodical edition evaluation", () => {
    ...
    it("reference supplement (Klammernzusatz) should display validation on blur and hide on focus", async () => {
        ...
        expect(referenceSupplementInput, "should not have error while typing").not.toHaveClass("has-error")
        await fireEvent.blur(referenceSupplementInput)
        expect(referenceSupplementInput, "should have error if empty and unfocused").toHaveClass("has-error")
        await fireEvent.focus(referenceSupplementInput)
        expect(referenceSupplementInput, "should hide error while editing").not.toHaveClass("has-error")
        await fireEvent.blur(referenceSupplementInput)
        expect(referenceSupplementInput, "should have error if empty and unfocused").toHaveClass("has-error")
    })
