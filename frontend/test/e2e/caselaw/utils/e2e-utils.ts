...
export async function openSearchWithFileNumberPrefix(
  ...
  if (kind && kind === Kind.PENDING_PROCEEDING) {
    await test.step("Wähle Tab 'Anhängige Verfahren' aus", async () => {
      await page.getByTestId("search-tab-pending-proceeding").click()
      await expect(page).toHaveURL(/pending-proceedings/)
    })
  }
  if (commonFileNumberPrefix) {
    await test.step("Befülle Suche mit Aktenzeichen-Prefix", async () => {
      await fillInput(page, "Aktenzeichen Suche", commonFileNumberPrefix)
    })
  }
}
