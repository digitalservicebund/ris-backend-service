...
test.describe("Editing and deleting references in periodical evaluation", () => {
  test("Rendering of citation input", async ({ page, edition }) => {
    const suffix = edition.suffix || ""
    await navigateToPeriodicalReferences(page, edition.id || "")

    await test.step("Citation shows selected prefix and suffix", async () => {
      await expect(page.getByLabel("Zitatstelle Präfix")).toHaveValue("2024, ")
  ...
  test("Add references to editon", async ({
    page,
    ...
    await test.step("Other docUnits can be added to an edition", async () => {
      ...
      await page.getByLabel("Treffer übernehmen").click()
      await expect(page.locator("[aria-label='Listen Eintrag']")).toHaveCount(4)
    })
  ...
  test("Preview of linked references", async ({
    page,
    edition,
    prefilledDocumentUnit,
  }) => {
  ...
  test("Periodical references are also added to docunit references", async ({
    page,
    prefilledDocumentUnit,
    context,
    edition,
    ...
    const suffix = edition.suffix || ""
    const fileNumber = prefilledDocumentUnit.coreData.fileNumbers?.[0] || ""
    await test.step("Add docunit to edition", async () => {
      await navigateToPeriodicalReferences(page, edition.id || "")
      await searchForDocUnitWithFileNumberAndDecisionDate(
        page,
        fileNumber,
  ...
  test("Editing of existing reference", async ({
    ...
    await test.step("When editing a reference, the citation is a single input containing the joined value of prefix, citation and suffix", async () => {
      ...
      await navigateToPeriodicalReferences(page, editionWithReferences.id || "")
      await expect(
