...
class LoadNormController(private val loadNormService: LoadNormUseCase) {
    ...
    private constructor(
        ...
        var isExpirationDateTemp: Boolean?,
        ...
        var digitalAccouncementDate: String?,
        ...
        companion object {
            fun fromUseCaseData(data: Norm): NormResponseSchema {
                ...
                return NormResponseSchema(
                    ...
                    data.risAbbreviation, data.risAbbreviationInternationalLaw,
                    data.documentNumber, data.divergentDocumentNumber, data.documentCategory, data.frameKeywords,
                    data.documentTypeName, data.documentNormCategory, data.documentTemplateName, data.providerEntity,
                    data.providerDecidingBody, data.providerIsResolutionMajority, data.participationType,
                    data.participationInstitution, data.leadJurisdiction, data.leadUnit, data.subjectFna,
                    data.subjectPreviousFna, data.subjectGesta, data.subjectBgb3,
                    data.officialShortTitle, data.officialAbbreviation, data.unofficialLongTitle,
                    data.unofficialShortTitle, data.unofficialAbbreviation,
                    encodeLocalDate(data.entryIntoForceDate), data.entryIntoForceDateState, encodeLocalDate(data.principleEntryIntoForceDate),
                    data.principleEntryIntoForceDateState, encodeLocalDate(data.divergentEntryIntoForceDate), data.divergentEntryIntoForceDateState,
                    encodeLocalDate(data.expirationDate), data.expirationDateState, data.isExpirationDateTemp, encodeLocalDate(data.principleExpirationDate),
                    data.principleExpirationDateState, encodeLocalDate(data.divergentExpirationDate), data.divergentExpirationDateState,
                    data.expirationNormCategory, encodeLocalDate(data.announcementDate), encodeLocalDate(data.publicationDate), encodeLocalDate(data.citationDate),
                    data.printAnnouncementGazette, data.printAnnouncementYear, data.printAnnouncementNumber,
                    data.printAnnouncementPage, data.printAnnouncementInfo, data.printAnnouncementExplanations,
                    data.digitalAnnouncementMedium, encodeLocalDate(data.digitalAccouncementDate), data.digitalAnnouncementEdition,
                    data.digitalAnnouncementYear, data.digitalAnnouncementPage, data.digitalAnnouncementArea,
                    data.digitalAnnouncementAreaNumber, data.digitalAnnouncementInfo, data.digitalAnnouncementExplanations,
                    data.euAnnouncementGazette, data.euAnnouncementYear, data.euAnnouncementSeries,
                    data.euAnnouncementNumber, data.euAnnouncementPage, data.euAnnouncementInfo,
                    data.euAnnouncementExplanations, data.otherOfficialAnnouncement, data.unofficialReference,
                    data.completeCitation, data.statusNote, data.statusDescription, encodeLocalDate(data.statusDate),
                    data.statusReference, data.repealNote, data.repealArticle, encodeLocalDate(data.repealDate),
                    data.repealReferences, data.reissueNote, data.reissueArticle, encodeLocalDate(data.reissueDate),
                    data.reissueReference, data.otherStatusNote, data.documentStatusWorkNote, data.documentStatusDescription,
                    encodeLocalDate(data.documentStatusDate), data.documentStatusReference, encodeLocalDate(data.documentStatusEntryIntoForceDate),
                    data.documentStatusProof, data.documentTextProof, data.otherDocumentNote, data.applicationScopeArea,
                    encodeLocalDate(data.applicationScopeStartDate), encodeLocalDate(data.applicationScopeEndDate), data.categorizedReference,
                    data.otherFootnote, data.validityRule, data.digitalEvidenceLink, data.digitalEvidenceRelatedData,
                    data.digitalEvidenceExternalDataNote, data.digitalEvidenceAppendix, data.referenceNumber,
                    data.europeanLegalIdentifier, data.celexNumber, data.ageIndicationStart, data.ageIndicationEnd,
                    data.definition, data.ageOfMajorityIndication, data.text
                )
            }
        }
    }
